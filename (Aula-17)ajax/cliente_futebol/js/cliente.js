...
function buscarClubes() {
    /* Versão antiga do ajax (usando XMLHttpRequest) */
    // Implementar uma requisição para futebol api
    var xhttp = new XMLHttpRequest();
    xhttp.open("GET" , URL_API);
    
    xhttp.onload = function(){
        // alert(xhttp.responseText);
        var json = xhttp.responseText; // pega a resposta do servidor no formato de um json
        var clubes = JSON.parse(json); // Tranforma o json em um array
        
        listarClubesTabela(clubes); // lista os clubes passando o array
    }
...
async function buscarClubesAwait() {
    /* Versão mais moderna (usando Promisse) */
    const options = { // Objeto JS
        method: "GET"
    };
    
    var resp = await fetch(URL_API, options); // fetch assincrono retorna a resposta
    var clubes = await resp.json(); // Pega o json assincrono
...
async function excluirClube(idClube) {
    ...
    const URL_EXCLUIR = URL_API + "/" + idClube;
    
    var resp = await fetch(URL_EXCLUIR, options);
    if(resp.status == 200){
        buscarClubesAwait();
    }
    else{
...
async function inserirClube() {
    ...
    const json = JSON.stringify(clube); // Converte clube pra JSON
    ...
    if(resp.status == 201){
        ...
        inpNome.value = ""; // Limpa os campos
        inpCidade.value = "";
        inpImagem.value = "";
        buscarClubesAwait(); // Busca os dados
    }
    else{
        /* Erro na inserção */
        var jsonErro = await resp.text();
